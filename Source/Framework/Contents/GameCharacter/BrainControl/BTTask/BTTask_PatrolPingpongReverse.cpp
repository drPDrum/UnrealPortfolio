#include "BTTask_PatrolPingpongReverse.h"
...
EBTNodeResult::Type UBTTask_PatrolPingpongReverse::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
...
void UBTTask_PatrolPingpongReverse::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	auto pBrainCtrl = Cast<ABrainController>(OwnerComp.GetAIOwner());
	auto pOwner = pBrainCtrl->GetOwner();
...
bool UBTTask_PatrolPingpongReverse::SetPatrolPoint(ABrainController* pBrainCtrl, AGameCharacter* pOwner, const bool& bForced)
{
	...
	{
		// EndPoint에 도달
		if (pSpawnInfo->IsPatrolBeginPoint())
			return true;
